...
define("admin/manage/category", [
  ...
  Category.init = function () {
    ...
    $("#category-selector").on("change", function () {
      ajaxify.go(
        "admin/manage/categories/" + $(this).val() + window.location.hash
      );
    });
    ...
    $("#category-settings input, #category-settings select").on(
      "change",
      function (ev) {
        modified(ev.target);
      }
    ...
    $("#save").on("click", function () {
      if (Object.keys(modified_categories).length) {
        socket.emit("admin.categories.update", modified_categories, function (
          err,
          result
        ) {
          if (err) {
    ...
    $(".purge").on("click", function (e) {
      ...
      Benchpress.parse(
        "admin/partials/categories/purge",
        {
    ...
    categorySelector.modal(categories, function (parentCid) {
      ...
      socket.emit("admin.categories.update", payload, function (err) {
        ...
        var parent = ajaxify.data.allCategories.filter(function (category) {
          return (
            category && parseInt(category.cid, 10) === parseInt(parentCid, 10)
          );
        });
        parent = parent[0];
        ...
        var buttonHtml =
          '<i class="fa ' + parent.icon + '"></i> ' + parent.name;
